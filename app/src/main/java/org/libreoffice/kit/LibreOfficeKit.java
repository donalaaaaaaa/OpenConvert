...
public final class LibreOfficeKit
{
    ...
    /**
     * OpenConvert 扩展：允许仅凭 Application Context 初始化（转换 Worker 无 Activity）。
     * apkFileOverride 提供 Office Pack zip 路径（LOKit 通过 mmap 读取其中的 assets/ 条目）。
     */
    public static synchronized void init(android.content.Context context, String apkFileOverride)
    {
        if (initializeDone) {
            return;
        }
        ...
        String apkFile = apkFileOverride != null ? apkFileOverride : applicationInfo.publicSourceDir;
    ...
    /**
     * OpenConvert 扩展：允许仅凭 Application Context 初始化（转换 Worker 无 Activity）。
     * 与 init(Activity) 等价，使用 packageResourcePath（与 Activity 版一致）。
     */
